...
void initCamera()
{
   double w = theJello1.GetWidth()*2;   
   double h = theJello1.GetHeight()*2;   
   double d = theJello1.GetDepth()*2;   
...
void onKeyboardCb(unsigned char key, int x, int y)
{
   ...
   if (key == ' ') theCamera.reset();
	   ...
	   theJello1.SetIntegrationType(JelloMesh::EULER);
	   theJello2.SetIntegrationType(JelloMesh2::EULER);
	   theJello3.SetIntegrationType(JelloMesh3::EULER);
   }
   else if (key == '9') {
	   theJello1.SetIntegrationType(JelloMesh::MIDPOINT);
	   theJello2.SetIntegrationType(JelloMesh2::MIDPOINT);
	   theJello3.SetIntegrationType(JelloMesh3::MIDPOINT);
   }
   else if (key == '0') {
	   theJello1.SetIntegrationType(JelloMesh::RK4);
	   theJello2.SetIntegrationType(JelloMesh2::RK4);
	   theJello3.SetIntegrationType(JelloMesh3::RK4);
   }
   else if (key == '>') isRunning = true;
   else if (key == '=') isRunning = false;
   else if (key == '<') {
	   theJello2.Reset(); theJello1.Reset(); theJello3.Reset();
   }
   else if (key == 'r') isRecording = !isRecording; if (isRecording) theFrameNum = 0;
   else if (key == '1') mask = theJello2.MESH;
   else if (key == '2') mask = theJello2.FORCES;
   else if (key == '3') mask = theJello2.NORMALS;
   else if (key == '4') mask = theJello2.STRUCTURAL;
   else if (key == '5') mask = theJello2.SHEAR;
   else if (key == '6') mask = theJello2.BEND;
