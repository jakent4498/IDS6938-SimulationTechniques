...
// Change to the same spring constants to show different integration methods
//double JelloMesh3::g_structuralKs = 4622;
double JelloMesh3::g_structuralKs = 1622;
double JelloMesh3::g_structuralKd = 8.0;
double JelloMesh3::g_attachmentKs = 0.0;
double JelloMesh3::g_attachmentKd = 0.0;
double JelloMesh3::g_shearKs = 1222.0;
//double JelloMesh3::g_shearKs = 3822.0;
//double JelloMesh3::g_shearKd = 15.50;
double JelloMesh3::g_shearKd = 7.50;
//double JelloMesh3::g_bendKs = 800.30;
double JelloMesh3::g_bendKs = 622.30;
//double JelloMesh3::g_bendKd = 8.30;
double JelloMesh3::g_bendKd = 6.30;
// penaltyKs of 600 works well for midpoint
// penaltyKs of 1000 to 1500
//works better for rk4
double JelloMesh3::g_penaltyKs = 1500.70;
double JelloMesh3::g_penaltyKd = 9.0;
double JelloMesh3::g_threshold = .3;
...
//				float x = -m_width*0.5f + wcellsize*i;
				float x = 2.0 -m_width*0.5f + wcellsize*i;
...
void JelloMesh3::DrawMesh(const vec3& eyePos)
{
    ...
    float red[4] = {0.2,0.3,0.0,0.9};
    float white[4] = {1.0,1.0,1.0,1.0};
    float pink[4] = {0.0,0.7,0.7,1.0};
...
//		pt.force -= pt.force;
//		pt.force = -1.0*JelloMesh3::g_penaltyKs*dist*normal - JelloMesh3::g_penaltyKd * (Dot(pt.velocity, pt.position) / dist)*normal;
...
//		pt.force = pt.force + JelloMesh3::g_penaltyKs*(dist*normal) + JelloMesh3::g_penaltyKd*(pt.velocity*normal)*normal;
	}
...
//		p.force = p.force - (2.0 * Dot(p.force, normal)) * normal * restitution;
		if (p.velocity*normal < 0)
			p.force = p.force + JelloMesh3::g_penaltyKs*(result.m_distance*normal) + JelloMesh3::g_penaltyKd*(p.velocity*normal)*normal;
...
bool JelloMesh3::SphereIntersection(Particle& p, World::Sphere* sphere, JelloMesh3::Intersection& result)
{
	vec3 sphereCenter = sphere->pos;
	double sphereRadius = sphere->r;
	vec3 p2Center = p.position - sphereCenter;

	if (p2Center.Length() - sphereRadius <= 0)
	{
		result.m_distance = sphereRadius - p2Center.Length();
		result.m_p = p.index;
		result.m_normal = p2Center.Normalize();
		result.m_type = JelloMesh3::CONTACT;
		return true;

	}
	if (p2Center.Length() - sphereRadius <= g_threshold)
	{
		result.m_distance = p2Center.Length() - sphereRadius;
		result.m_p = p.index;
		result.m_normal = p2Center.Normalize();
		result.m_type = JelloMesh3::COLLISION;
		return true;
...
/*	vec3 cylNorm = cylinderAxis.Normalize();
	...
	*/
	...
	if (dist - cylinderRadius <= 0.0)
	{
		...
		if (pt > cylinderAxis.Length()) return false;
...
void JelloMesh3::EulerIntegrate(double dt)
{
    // TODO
	/*
