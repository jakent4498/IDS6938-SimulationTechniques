...
double JelloMesh2::g_structuralKs = 1622;
double JelloMesh2::g_structuralKd = 8.0; 
double JelloMesh2::g_attachmentKs = 0.0;
double JelloMesh2::g_attachmentKd = 0.0;
//double JelloMesh2::g_shearKs = 800.0;
double JelloMesh2::g_shearKs = 1222.0;
//double JelloMesh2::g_shearKd = 15.50;
double JelloMesh2::g_shearKd = 7.50;
//double JelloMesh2::g_bendKs = 800.30;
double JelloMesh2::g_bendKs = 622.30;
//double JelloMesh2::g_bendKd = 8.30;
double JelloMesh2::g_bendKd = 6.30;
// penaltyKs of 600 works well for midpoint
// penaltyKs of 1000 to 1500
//works better for rk4
double JelloMesh2::g_penaltyKs = 1500.70;
double JelloMesh2::g_penaltyKd = 9.0;
double JelloMesh2::g_threshold = .3;

JelloMesh2::JelloMesh2() :     
    m_integrationType(JelloMesh2::EULER), m_drawflags(MESH | STRUCTURAL),
...
void JelloMesh2::DrawMesh(const vec3& eyePos)
{
    ...
    float red[4] = {0.0,0.0,4.4,0.7};
    float white[4] = {1.0,1.0,1.0,1.0};
    float pink[4] = {0.0,0.0,2.2,1.0};
...
//		pt.force -= pt.force;
//		pt.force = -1.0*JelloMesh2::g_penaltyKs*dist*normal - JelloMesh2::g_penaltyKd * (Dot(pt.velocity, pt.position) / dist)*normal;
...
//		pt.force = pt.force + JelloMesh2::g_penaltyKs*(dist*normal) + JelloMesh2::g_penaltyKd*(pt.velocity*normal)*normal;
	}
...
//		p.force = p.force - (2.0 * Dot(p.force, normal)) * normal * restitution;
		if (p.velocity*normal < 0)
			p.force = p.force + JelloMesh2::g_penaltyKs*(result.m_distance*normal) + JelloMesh2::g_penaltyKd*(p.velocity*normal)*normal;
...
bool JelloMesh2::SphereIntersection(Particle& p, World::Sphere* sphere, JelloMesh2::Intersection& result)
{
	...
	if (p2Center.Length() - sphereRadius <= 0)
	{
		...
		result.m_type = JelloMesh2::CONTACT;
		return true;
	...
	if (p2Center.Length() - sphereRadius <= g_threshold)
	{
		...
		result.m_p = p.index;
		result.m_normal = p2Center.Normalize();
		result.m_type = JelloMesh2::COLLISION;
		return true;
...
void JelloMesh2::EulerIntegrate(double dt)
{
    // TODO
	/*
